...
object RequestHttp:
  ...
  sealed class Method(protected var _okStatus: Int):
    ...
    def send[A](dest: String, body: A = null, headers: List[(String, String)] = Nil)(implicit writer: A => String = { (a: A) => if a != null then a.toString else null }): Future[Response] =
    ...
    new Promise[Response]((resolve, reject) => {
      ...
      request.onreadystatechange = (_: Event) => {
        ...
        if request.readyState == XMLHttpRequest.DONE then {
          ...
          if okStatus(request.status) then {
            resolve(resp)
          } else {
