...
import bon.jo.html.SimpleView._
...
object SimpleView:
  ...
  def sel[A](elemnts: Iterable[A])(using kv: (A => String, A => String)): Select =
  ...
  def sv[A](using elemnts: List[A], v: A => String): Select = sel(elemnts)(using (k => k.toString, a => v(a)))

  def s[A](using elemnts: List[A]): Select = sel(elemnts)(using (k => k.toString, a => a.toString))
