...
object DomShell {
  ...
  def inputXml(name: String, label: String, value: Any = ""): Elem = <div class="form-group">
    <label for={s"" + name} class="form-label">{label}</label> <input class="form-control" name={s"" + name} id={s"" + name} placeholder={""+label} value={"" + value}/>
  </div>

  def inputHtml(name: String, label: String, value: Any = ""): Div = BridgeXmlHtml.toElement(inputXml(name,label,value))

  def button(id: String,text : String, class_ : String = ""): Elem =  <button id={""+id} class={"btn btn-primary"+class_} type="button">{text}</button>
  object $c{
